...
$shell_props_id = 'shell-navigation-props';
$legacy_auto_shell_props_id = 'legacy-auto-shell-props';
$active_module_id = $request->getUri()->getSegment(1);
$active_section = $request->getUri()->getSegment(2);
$react_modules = array_map(static fn($module): array => [
    ...
    'active'  => $module->module_id == $request->getUri()->getSegment(1),
], $allowed_modules);

$shell_props = [
    'company'           => (string) $config['company'],
    'currentTime'       => date($config['dateformat'] . ' ' . $config['timeformat']),
    'userName'          => trim($user_info->first_name . ' ' . $user_info->last_name),
    'changePasswordUrl' => site_url("home/changePassword/$user_info->person_id"),
    'logoutUrl'         => site_url('home/logout'),
    'homeUrl'           => site_url(),
    'modules'           => $react_modules,
];
$legacy_auto_shell_props = [
    'title'   => $active_module_id ? lang('Module.' . $active_module_id) : (string) $config['company'],
    'section' => $active_section ? ucfirst(str_replace('_', ' ', $active_section)) : null,
];
?>
...
<body>
    <div class="wrapper">
        <div data-react-root="shell-navigation" data-props-id="<?= esc($shell_props_id, 'attr') ?>">
            <div class="topbar">
                <div class="container">
                    <div class="navbar-left">
                        <div id="liveclock"><?= esc($shell_props['currentTime']) ?></div>
                    </div>
        ...
        <script id="<?= esc($shell_props_id, 'attr') ?>" type="application/json"><?= json_encode($shell_props, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_THROW_ON_ERROR) ?></script>
